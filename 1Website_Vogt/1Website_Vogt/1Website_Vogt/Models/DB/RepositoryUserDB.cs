...
namespace _1Website_Vogt.Models.DB {
    public class RepositoryUserDB {
        private string connectionString = "Server=localhost;database=myindigo;user=root;password=";
        ...
        public async Task<bool> InsertAsync(User user) {
            if (this.connection?.State == ConnectionState.Open) {
                ...
                cmdInsert.CommandText = "insert into users values(null, @vorname, @nachname," +
                    "@ort, @strasse, @hausnummer, @postleitzahl, @email, @zahlung);";
                ...
                DbParameter paramZ = cmdInsert.CreateParameter();
                paramH.ParameterName = "zahlung";
                paramH.DbType = DbType.Int32;
                paramH.Value = user.Zahlung;
                ...
                cmdInsert.Parameters.Add(paramZ);
        ...
        public async Task<bool> InsertGerichtAsync(Gericht gericht) {
            if (this.connection?.State == ConnectionState.Open) {

                DbCommand cmdInsert = this.connection.CreateCommand();
                cmdInsert.CommandText = "insert into bestellung values(null, @vorname, @nachname);";

                DbParameter paramG = cmdInsert.CreateParameter();
                paramG.ParameterName = "gerichtName";
                paramG.DbType = DbType.String;
                paramG.Value = gericht.gerichtName;

                DbParameter paramPr = cmdInsert.CreateParameter();
                paramPr.ParameterName = "preis";
                paramPr.DbType = DbType.Double;
                paramPr.Value = gericht.preis;
        ...
        internal Task<bool> InsertAsync(Gericht gerichtData) {
